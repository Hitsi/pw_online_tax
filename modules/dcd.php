...
/*
 * Добавление информации в таблицу ДЦД
 */
...
if (!in_array("dcd", $_SESSION['SessinLogin'])) {
    header("Location: /404/");
    exit();
}
/*
...
function Add($Form) {
    ...
    if (!isset($Form["users"]) or $Form["users"] == "") {
        ...
        $formball = $Form["ball"];
        try {
            $sql = "select count(uid) as count from " . $prefixdb . "dcd where date=:date";
        ...
        if ((isset($result) and $result["count"] > 0) and (isset($Form["update"]) and $Form["update"] == "on")) {
            try {
                $sql = "delete from " . $prefixdb . "dcd where date=:date";
                $sth = $DBH->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                $sth->execute(array(":date" => $date));
                $sql = "delete from " . $prefixdb . "dcd_profit where date=:date";
                $sth = $DBH->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                $sth->execute(array(":date" => $date));
                $sql = "insert into " . $prefixdb . "dcd_profit (date,cost,online,users) values (:date,:cost,:online,:users)";
                $sth = $DBH->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                $sth->execute(array(":date" => $date, ":cost" => $cost, ":online" => $online_profit, ":users" => $users_profit));
                $sql = "insert into " . $prefixdb . "dcd (uid,date,ball,text) values (:uid,:date,:ball,:text)";
                $sth = $DBH->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                foreach ($ball as $key => $value) {
...
function Load($date) {
    ...
    if (!isset($date) or empty($date)) {
        ...
        try {
            $sql = "select *, count(date) as count from " . $prefixdb . "dcd_profit where date=:date";
            $sth = $DBH->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
            $sth->execute(array(":date" => $date));
            $result = $sth->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
...
function Delete($date) {
    ...
    if (!isset($date) or empty($date)) {
        ...
        try {
            $sql = "select *, count(date) as count from " . $prefixdb . "dcd where date=:date";
            $sth = $DBH->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
            $sth->execute(array(":date" => $date));
            $result = $sth->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
